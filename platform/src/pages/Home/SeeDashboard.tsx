...
import api, { User, Student, Therapist} from "@/utils/api";
...
export default function SeeDashboard() {
  ...
  const [studentCount, setStudentCount] = useState(0); // Add state for student count
  ...
  const [therapistCount, setTherapistCount] = useState(0); // Add state for student count
  ...
  useEffect(() => {
    ...
    api.students.list().then((v) => {
      ...
      api.therapists.list().then((v) => {
        ...
        setTherapistCount(valTher.length); // Update the therapist count
      });
